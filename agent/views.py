...
from common.models import (
    ...
    TrackingEvent, TourShipment, InvoiceLine
)
...
def create_shipment(request):
    ...
    if request.method == 'POST':
        ...
        base_rate = Decimal('10.0')  # Base delivery rate
        weight_rate = Decimal('2.0')  # Per kg
        volume_rate = Decimal('5.0')  # Per cubic meter
...
class ShipmentDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['tracking_history'] = self.object.tracking_events.all().order_by('-timestamp')
        return context
...
def create_invoice(request):
    ...
    if request.method == 'POST':
        ...
        if not shipment_ids:
            messages.error(request, 'Please select at least one shipment.')
            return redirect('agent:create_invoice')

        invoice_date_str = request.POST.get('invoice_date')
        if invoice_date_str:
            issue_date = timezone.datetime.strptime(invoice_date_str, '%Y-%m-%d').date()
        else:
        ...
        invoice = Invoice.objects.create(
            ...
            due_date=issue_date + timedelta(days=30),  # Default 30 days due
        ...
        # Calculate HT based on TTC and TVA rate
        # TTC = HT * (1 + rate/100)  =>  HT = TTC / (1 + rate/100)
        tva_multiplier = Decimal('1') + (invoice.tva_rate / Decimal('100'))
        invoice.amount_ht = total_ttc / tva_multiplier
        invoice.save()  # save() will verify and recalc TVA/TTC to be exact
...
def get_client_shipments(request):
    ...
    shipments = Shipment.objects.filter(
        client_id=client_id,
        status='delivered'
    ).exclude(invoiceline__isnull=False)  # Not already invoiced
